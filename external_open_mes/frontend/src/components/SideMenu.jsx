...
const SideMenu = ({ isOpen, isStaffOrSuperuser, onVersionClick, onLinkClick, onLogout }) => {
  ...
  return (
    <nav id="menu-bar" className={isOpen ? 'open' : ''}>
      ...
      <Link to="/production/plan-gantt" className="menu-subcategory-link" onClick={onLinkClick}>生産計画検索 (ガントチャート)</Link>
      <Link to="/production/worker-interface" className="menu-subcategory-link" onClick={onLinkClick}>作業者インターフェース</Link>
